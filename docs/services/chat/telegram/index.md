...
!!! tip
    You can use the `circa10a/shoutrrr` image in docker to run it without download/installing the `shoutrrr` CLI using:
    ```
    docker run --rm -it circa10a/shoutrrr generate telegram
    ```
...
The bot should be constantly online, unless it's usage exceeds the free tier on GCP. It's source is available at [github.com/circa10a/shoutrrrbot](https://github.com/circa10a/shoutrrrbot).
